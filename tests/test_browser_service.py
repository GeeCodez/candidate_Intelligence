...
def test_select_relevant_sources_only_uses_github_sources():
    ...
    sources = [
        SimpleNamespace(
            url="https://linkedin.com/in/example", source_type="other", title="LinkedIn"
        ),
        SimpleNamespace(
            url="https://github.com/example", source_type="github", title="GitHub"
        ),
        SimpleNamespace(
...
def test_select_relevant_sources_rejects_github_non_profile_or_repo_urls():
    claim = SimpleNamespace(claim="Candidate has experience with Django.")
    requirement = SimpleNamespace(name="Django", description="Build Django APIs")
    sources = [
        SimpleNamespace(
            url="https://github.com/topics/django", source_type="github", title="Topic"
        ),
        SimpleNamespace(
            url="https://github.com/example/project", source_type="github", title="Repo"
        ),
        SimpleNamespace(
    ...
    assert [source.url for source in selected] == ["https://github.com/example/project"]
